...
from i18n import ansiprint, capture_output, captured_text
import spelling_correction
from ai_parser import parse_command, parse_target, parse_ai_command
import ai_director
...
class Char():
    ...
    def turn(self,turn_counter):
        ...
        if self.turnMoment == 0:
            ...
            self.turnMoment = 1
            entities.visit_ai_turn(self, turn_counter)
        else:
        ...
        while True:
            i = 0
            try:
                self.print_combat_panel()
                plan_input = input("\n请直接描述你想做的事（也可输入编号）：\n")
                self.pending_target = None
                if plan_input.strip().isdigit():
                    command = None
                else:
                    model_payload = ai_director.interpret_command(
                        entities._ai_combat_state(self, turn_counter), plan_input
                    )
                    command = parse_ai_command(
                        model_payload,
                        self.hand,
                        self.potionBag,
                    )
                    if model_payload is None:
                        command = parse_command(plan_input, self.hand, self.potionBag)
                if command is not None and command["kind"] == "play_card":
                    plan = command["index"]
                    self.pending_target = command.get("target_hint")
                elif command is not None and command["kind"] == "use_potion":
                    plan = len(self.hand) + command["index"]
                elif command is not None and command["kind"] == "end_turn":
                    plan = len(self.hand) + len(self.potionBag)
                elif command is not None and command["kind"] == "show_relics":
                    plan = len(self.hand) + len(self.potionBag) + 1
                elif command is not None and command["kind"] == "show_drawpile":
                    plan = len(self.hand) + len(self.potionBag) + 2
                elif command is not None and command["kind"] == "show_discardpile":
                    plan = len(self.hand) + len(self.potionBag) + 3
                elif command is not None and command["kind"] == "show_exhaustpile":
                    plan = len(self.hand) + len(self.potionBag) + 4
                elif command is not None and command["kind"] == "save":
                    plan = len(self.hand) + len(self.potionBag) + 5
                elif command is not None and command["kind"] == "inspect":
                    self.explainer_function(command["query"])
                    continue
                else:
                    try:
                        plan = int(plan_input) - 1
                    except ValueError:
                        if command is not None and command["kind"] == "inspect":
                            self.explainer_function(command["query"])
                        else:
                            ansiprint("无法识别这个操作。请输入数字，或输入“打出猛击”“结束回合”等指令。")
                        continue
                ...
                action_before = None
                action_output = []
                action_label = plan_input.strip()

                if plan in range(len(self.hand)):
                    action_kind = "play_card"
                    action_before = entities._ai_combat_state(self, turn_counter)
                    if action_label.isdigit():
                        action_label = f"打出 {self.hand[plan].get('Name')}"
                    with capture_output() as action_output:
                    ...
                    if plan < len(self.hand) + len(self.potionBag):
                        ...
                        action_before = entities._ai_combat_state(self, turn_counter)
                        if action_label.isdigit():
                            action_label = f"使用 {self.potionBag[potion_index].get('Name')}"
                        with capture_output() as action_output:
                ...
                battle_over = not any(enemy.alive for enemy in entities.list_of_enemies)
                if action_kind is not None:
                    if battle_over:
                        with capture_output() as battle_output:
                            self.end_of_battle_effects()
                        action_output.extend(battle_output)
                    self.present_action_summary(
                        action_before,
                        action_label,
                        action_output,
    ...
    def print_combat_panel(self):
        ansiprint("\n<light-blue>━━━━━━━━━━ 战斗状态 ━━━━━━━━━━</light-blue>")
        self.show_status()
        ansiprint("<red>敌人</red>")
        self.showEnemies(skip=False, numbers=True)
        ansiprint("<blue>手牌与药水</blue>")
        self.print_hand_and_potions()
        ansiprint("<light-blue>━━━━━━━━━━━━━━━━━━━━━━━━━━━━</light-blue>")
    ...
    def present_action_summary(
        ...
        after = entities._ai_combat_state(self, helping_functions.turn_counter)
        feedback = captured_text(action_output)
        ansiprint("<light-blue>行动结果：</light-blue>")
        summary, _ = ai_director.generate_action_summary(
            before,
            after,
            player_command,
            feedback,
    ...
    def print_hand_and_potions(self):
        ...
        ansiprint(f"{hand_and_potion_length + 1}. 结束回合")
        ansiprint(f"{hand_and_potion_length + 2}. 查看遗物")
        ansiprint(f"{hand_and_potion_length + 3}. 查看抽牌堆")
        ansiprint(f"{hand_and_potion_length + 4}. 查看弃牌堆")
        ansiprint(f"{hand_and_potion_length + 5}. 查看消耗牌堆")
        ansiprint(f"{hand_and_potion_length + 6}. 保存游戏")
        ansiprint("也可以直接描述行动，例如‘打出猛击’、‘对左边的敌人使用猛击’或‘结束回合’。")
    ...
    def cardsDrawnEffectCheck (self,cardsDrawn):
        cardsDrawn = min(cardsDrawn, len(self.hand))
        i = 0
        agonyCount = 0
        agonyPlusCount = 0
        evolveCount = 0
    ...
    def choose_enemy(self):
        ...
        if len (entities.list_of_enemies) > 0:
            ...
            while True:
                try:
                    if self.randomTarget == False:
                        target_input = getattr(self, "pending_target", None)
                        self.pending_target = None
                        if target_input is None:
                            target_input = input(
                                f"\n请选择目标（1-{len(entities.list_of_enemies)}，输入 "
                                f"{len(entities.list_of_enemies) + 1} 跳过；也可输入敌人名称、左边的敌人或‘攻击右边’）\n"
                            )
                        target = parse_target(target_input, entities.list_of_enemies)
                        if target is None:
                            ansiprint("无法识别目标。请输入编号、敌人名称，或自然语言描述，例如‘攻击左边的敌人’。")
                            continue
                    else:
                    ...
                    if target in range(len(entities.list_of_enemies)):
                        ...
                        ansiprint(
                            f"没有这个目标。请输入 1-{len(entities.list_of_enemies)}、敌人名称，"
                            f"或自然语言描述，例如‘攻击左边的敌人’；输入 {len(entities.list_of_enemies) + 1} 跳过。"
                        )
                        continue
                except ValueError:
                    if len(target) > 0:
    ...
    def enemy_preview(self,index,spotWeaknessCheck=False):
        ...
        if self.runicDome == 1 and spotWeaknessCheck == False:
            ...
            if type(entities.list_of_enemies[index].move) == int:
                ...
                damage = self.determine_damage_to_character(
                    int(entities.list_of_enemies[index].move.split(" ")[1]) + additionalDamage,
                    index,
                )
    ...
    def determine_damage_to_enemy(self,index):
        ...
        return f"目标将受到 <red>{attack} 点伤害</red>（来自 <red>{self.card_in_play.get('Name')}</red>）"
