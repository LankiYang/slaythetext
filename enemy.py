...
class Enemy():
    ...
    def determine_choice(self,turn_counter):
        ...
        if self.intention_logic[0][0] == "Random":
            ...
            sequence = self.intention_logic[1]
            if sequence and isinstance(sequence[0], list):
                sequence = sequence[0]
            if not sequence or not self.intentions:
                choice = self.intentions[0]
            else:
                sequence_index = sequence[(turn_counter - 1) % len(sequence)]
                choice = self.intentions[sequence_index % len(self.intentions)]
            ...
            sequence = self.intention_logic[1] if len(self.intention_logic) > 1 else []
            if sequence and isinstance(sequence[0], list):
                sequence = sequence[0]
            if not self.intentions:
                choice = None
            elif not sequence:
                choice = self.intentions[(turn_counter - 1) % len(self.intentions)]
            else:
                sequence_index = sequence[(turn_counter - 1) % len(sequence)]
                choice = self.intentions[sequence_index % len(self.intentions)]
        ...
        if type(action) == int:
            ...
            if "Block" in action:
                ...
                if self.name == "<red>Book of Stabbing</red>":
                    while i < helping_functions.turn_counter+1:
                        stabbingCheck = entities.active_character[0].health
                ...
                while i < int(int(action.split(" ")[1].split("*")[0])):
                        entities.active_character[0].add_CardToDiscardpile({"Name": "Burn +", "DiscardDamage": 4, "Type": "Status", "Rarity": "Enemy","Upgraded": True,"Owner":"The Spire","Info":"<RED>Unplayable</RED>. At the end of your turn, take <red>4 damage</red>."})
                        i += 1
                ansiprint(entities.active_character[0].name,"now has",action.split(" ")[1].split("*")[0], entities.cards["Burn +"]["Name"], "in the discard pile.")
                self.tracker = 1
    ...
    def receive_recoil_damage(self,attack_damage,orbDamage=False):
        if orbDamage == True and self.lockon > 0:
            attack_damage += attack_damage * 0.5
            attack_damage = math.floor(attack_damage)
